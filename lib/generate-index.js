...
module.exports.register = (eventEmitter) => {
  eventEmitter.on('beforeMapSite', ({ playbook, pages }) => {
    indexer.doGenerateIndex(playbook, pages)
  })

  eventEmitter.on('afterMapSite', (playbook, siteFiles) => {
    indexer.doCreateIndexFile(playbook, siteFiles)
  })

  const indexer = {
    index: undefined,

    doGenerateIndex: (playbook, pages) => {
      this.index = generateIndex(playbook, pages)
    },

    doCreateIndexFile: (playbook, siteFiles) => siteFiles.push(createIndexFile(this.index))
  }
}
...
module.exports._internal_generateIndex = generateIndex
module.exports._internal_createIndexFile = createIndexFile
